...
**HALF-DONE** Goal difference [total, home, away] - All Seasons Combined + For Each Season 2013-2019
...
all_seasons %>%
  ...
  mutate(TotalShotsTarget = ShotsTargetAway + ShotsTargetHome) 
```
...
all_seasons %>%
  ...
  mutate(TotalGoalsFor = GoalsForAway + GoalsForHome) 
```
...
all_seasons %>%
  ...
  mutate(TotalGoalsAgainst = GoalsAgainstAway + GoalsAgainstHome) 
```
...
all_seasons %>%
  ...
  select(season, Game, GoalsForHalfTime) %>%
  pivot_wider(names_from = Game,
...
all_seasons %>%
  ...
  select(season, Game, GoalsAgainstHalfTime) %>%
  pivot_wider(names_from = Game,
...
all_seasons %>%
  ...
  mutate(GoalsFor = ifelse(Game == "Home", FTHG, FTAG)) %>%
  mutate(GoalsAgainst = ifelse(Game == "Home", FTAG, FTHG)) %>%
  select(season, Game, GoalsFor, GoalsAgainst) %>%
