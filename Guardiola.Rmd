...
GoalsForTable %>% 
  group_by(Manager) %>% 
  mutate(AverageManagerGoals = mean(TotalGoalsFor), AverageManagerGPG = sum(TotalGoalsFor)/ (38*3)) %>% 
  ungroup() %>% 
  group_by(season) %>% 
  mutate(AverageSeasonGPG = TotalGoalsFor/38) %>% 
  select(season, Manager, AverageManagerGoals, AverageManagerGPG, AverageSeasonGPG) -> AverageGoalsForTable
...
GoalsAgainstTable %>% 
  group_by(Manager) %>% 
  mutate(AverageManagerGoalsAgainst = mean(TotalGoalsAgainst), AverageManagerGAPG = sum(TotalGoalsAgainst)/ (38*3)) %>% 
  ungroup() %>% 
  group_by(season) %>% 
  mutate(AverageSeasonGAPG = TotalGoalsAgainst/38) %>% 
  select(season, Manager, AverageManagerGoalsAgainst, AverageManagerGAPG, AverageSeasonGAPG) -> AverageGoalsAgainstTable
...
Man_City_Stats_PL_Website %>%
  group_by(Manager) %>%
  mutate(AverageSeasonTacklesManager = mean(Tackles), AverageGameTacklesManager = mean(Tackles)/38, AverageSeasonInterceptionsManager = mean(Interceptions), AverageGameInterceptionsManager = mean(Interceptions)/38) %>% 
  select(Season, Manager, AverageSeasonTacklesManager, AverageGameTacklesManager, AverageSeasonInterceptionsManager, AverageGameInterceptionsManager) -> AverageDefenseTable

AverageDefenseTable %>%
  gt(rowname_col = "Season") %>%
  tab_stubhead("Season") %>%
  tab_header(title = md("**Defensive Statistics**"))
```
