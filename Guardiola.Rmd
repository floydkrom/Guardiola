...
PointsTable %>%
  ...
  geom_col() +
  geom_text(aes(label = TotalPoints), position = position_dodge(width = 0.9), color = "White", vjust = -0.4) +
...
Man_City_Stats_PL_Website %>%
  ...
  geom_text(aes(label = `Passes per match`), 
            position = position_dodge(width = 0.9), 
            color = "White", vjust = -0.4) +
  labs(title = "", x = "", y = "") +
  scale_fill_manual(values = c("#B9D6F2", "#0353A4"), labels = c("Pellegrini", "Guardiola")) +
  removeGrid(x = TRUE, y = TRUE) +
  theme(plot.title.position = "plot", plot.background = element_rect(fill = "#061A40"), plot.title = element_markdown(size = 15, face = "bold", color = "White"), axis.text = element_text(color = "White"), axis.title.x.top = element_text(color = "White"), strip.text = element_text(size = 12, face = "italic", color = "#061A40"), strip.background.x = element_rect(fill = "grey"))
```
